...
# ===================================================================
# === PARAMETERS TO TUNE FOR ATTA SOURCE REGIME ===
# ===================================================================
N_G = 400
N_T = 800  # Higher frequency requires good temporal resolution

# --- CORE PARAMETER CHANGES ---
# Values inspired by Robertz (2023) and Ritterbusch (2009)
FREQUENCY = 15e6              # CCP frequency (can be set to 0 if VOLTAGE=0)
VOLTAGE = 0.0                  # Set to 0.0 to ensure heating is only from the ICP term
L = 0.062                      # Characteristic length (e.g., Helical Resonator shield diameter)
PRESSURE = 5e-3            # Pressure in Pascal (equivalent to ~1e-5 mbar)
TEMPERATURE = 150.0            # Cold gas temp consistent with LN2-dewar operation

# --- PARTICLE & WEIGHTING ---
WEIGHT = 2e7                # A good starting guess, tune as needed
N_INIT = 10_000                  # Initial seed particles
MAX_PARTICLES_FACTOR = 100
MAX_E_PARTICLES = N_INIT * MAX_PARTICLES_FACTOR
MAX_I_PARTICLES = N_INIT * MAX_PARTICLES_FACTOR
ELECTRODE_AREA = PI * (L/2)**2 # Assume circular cross-section for weighting

# --- SOLVER AND HEATING METHOD ---
USE_MAXWELL_SOLVER = False  # Use Poisson solver + Fictitious ICP field

# --- ICP "Fictitious" Bulk Heating Parameters ---
# This is your main tuning knob for power deposition in this 1D model
E_INDUCED_AMPLITUDE = 11_000.0 # [V/m] Effective heating field amplitude. Tune this!
RF_FREQ_ICP = 150e6          # [Hz] Driving frequency of the inductive heating
OMEGA_ICP = TWO_PI * RF_FREQ_ICP
# ===================================================================


# Additional (derived) constants
PERIOD = 1.0 / RF_FREQ_ICP if RF_FREQ_ICP > 0 else 1.0 # Avoid division by zero
...
def init_particles(nseed):
    print(">> eduPIC: Initializing with a WARM, UNIFORM plasma...")
    N_e_arr[0] = nseed
    N_i_arr[0] = nseed
    ...
    # Maxwellian velocity distribution with an initial temperature (e.g., 2 eV)
    initial_electron_temp_eV = 2
    v_thermal_e = math.sqrt(K_BOLTZMANN * (initial_electron_temp_eV / (K_BOLTZMANN/E_CHARGE)) / E_MASS)
    
    # Generate random velocities from a normal distribution
    vx_e[:nseed] = RNG.normal(0.0, v_thermal_e, size=nseed)
    vy_e[:nseed] = RNG.normal(0.0, v_thermal_e, size=nseed)
    vz_e[:nseed] = RNG.normal(0.0, v_thermal_e, size=nseed)

    # --- Ions ---
    # Uniformly distributed in space
    x_i[:nseed] = RNG.random(nseed) * L
    
    # Maxwellian velocity distribution at the background gas temperature
    v_thermal_i = math.sqrt(K_BOLTZMANN * TEMPERATURE / AR_MASS)
    
    vx_i[:nseed] = RNG.normal(0.0, v_thermal_i, size=nseed)
    vy_i[:nseed] = RNG.normal(0.0, v_thermal_i, size=nseed)
    vz_i[:nseed] = RNG.normal(0.0, v_thermal_i, size=nseed)
...
@numba.njit(fastmath=True, cache=True, nogil=True)
def solve_Maxwell_FDTD_jit(
    efield_x, efield_y, efield_z,
    bfield_x, bfield_y, bfield_z,
    jcurrent_x, jcurrent_y, jcurrent_z,
    tt, N_G_local, DX_local, DT_local,
    EPSILON0_local, MU0_local, OMEGA_ICP_local, B_AMP_local
):
    """
    1D FDTD Maxwell solver with external RF antenna for ICP.
    
    For 1D (x-direction) with out-of-plane fields (y,z):
    - ∂Ey/∂t = -(1/ε0) * Jy + (1/μ0ε0) * ∂Bz/∂x
    - ∂Ez/∂t = -(1/ε0) * Jz - (1/μ0ε0) * ∂By/∂x
    - ∂By/∂t = -∂Ez/∂x  (Faraday)
    - ∂Bz/∂t = ∂Ey/∂x   (Faraday)
    
    External antenna adds oscillating By field.
    """
    
    c_sq = 1.0 / (MU0_local * EPSILON0_local)  # c^2
    
    # External antenna: Add oscillating B-field in center region
    # Localized to antenna position (center of domain)
    antenna_start = int(0.4 * N_G_local)
    antenna_end = int(0.6 * N_G_local)
    
    for i in range(antenna_start, antenna_end):
        # Prescribed oscillating By (antenna field)
        B_external = B_AMP_local * math.sin(OMEGA_ICP_local * tt)
        bfield_y[i] += 0.1 * DT_local * (B_external - bfield_y[i]) / DT_local
        # This is a soft drive; adjust coupling strength (0.1) as needed
    
    # Update B-field (Faraday's law: ∂B/∂t = -∇×E)
    # ∂By/∂t = -∂Ez/∂x
    # ∂Bz/∂t = ∂Ey/∂x
    
    for i in range(1, N_G_local - 1):
        dEz_dx = (efield_z[i+1] - efield_z[i-1]) / (2.0 * DX_local)
        dEy_dx = (efield_y[i+1] - efield_y[i-1]) / (2.0 * DX_local)
        
        bfield_y[i] += -DT_local * dEz_dx
        bfield_z[i] += DT_local * dEy_dx
    
    # Boundary conditions for B (simple: extrapolate or set to zero)
    bfield_y[0] = bfield_y[1]
    bfield_y[N_G_local-1] = bfield_y[N_G_local-2]
    bfield_z[0] = bfield_z[1]
    bfield_z[N_G_local-1] = bfield_z[N_G_local-2]
    
    # Update E-field (Ampère-Maxwell: ∂E/∂t = c²∇×B - J/ε0)
    # ∂Ey/∂t = c² * ∂Bz/∂x - Jy/ε0
    # ∂Ez/∂t = -c² * ∂By/∂x - Jz/ε0
    
    for i in range(1, N_G_local - 1):
        dBz_dx = (bfield_z[i+1] - bfield_z[i-1]) / (2.0 * DX_local)
        dBy_dx = (bfield_y[i+1] - bfield_y[i-1]) / (2.0 * DX_local)
        
        efield_y[i] += DT_local * (c_sq * dBz_dx - jcurrent_y[i] / EPSILON0_local)
        efield_z[i] += DT_local * (-c_sq * dBy_dx - jcurrent_z[i] / EPSILON0_local)
    
    # Ex update (from electrostatic Poisson, not changed in simple 1D EM)
    # In full EM, would also update Ex, but for ICP we keep it from Poisson
    
    # Boundary conditions for E (absorbing or zero)
    efield_y[0] = 0.0
    efield_y[N_G_local-1] = 0.0
    efield_z[0] = 0.0
    efield_z[N_G_local-1] = 0.0

@numba.njit(fastmath=True, cache=True, nogil=True)
def deposit_current_jit(
    jcurrent_x, jcurrent_y, jcurrent_z,
    N_e_count, N_i_count,
    x_e, vx_e, vy_e, vz_e,
    x_i, vx_i, vy_i, vz_i,
    N_G_local, DX_local, WEIGHT_local, E_CHARGE_local, ELECTRODE_AREA_local
):
    """
    Deposit current density J = Σ q_i * v_i * weight onto grid.
    """
    # Reset currents
    jcurrent_x[:] = 0.0
    jcurrent_y[:] = 0.0
    ...
    DV = ELECTRODE_AREA_local * DX_local
    FACTOR_W = WEIGHT_local / DV
    INV_DX_local = 1.0 / DX_local
    
    # Electron contribution (negative charge)
    for k in range(N_e_count):
        pos_norm = x_e[k] * INV_DX_local
        p = int(math.floor(pos_norm))
        rmod = pos_norm - p
        
        if 0 <= p < N_G_local - 1:
            j_contrib_x = -E_CHARGE_local * vx_e[k] * FACTOR_W
            j_contrib_y = -E_CHARGE_local * vy_e[k] * FACTOR_W
            j_contrib_z = -E_CHARGE_local * vz_e[k] * FACTOR_W
            
            jcurrent_x[p] += (1.0 - rmod) * j_contrib_x
            jcurrent_x[p+1] += rmod * j_contrib_x
            
            jcurrent_y[p] += (1.0 - rmod) * j_contrib_y
            jcurrent_y[p+1] += rmod * j_contrib_y
            
            jcurrent_z[p] += (1.0 - rmod) * j_contrib_z
            jcurrent_z[p+1] += rmod * j_contrib_z
    
    # Ion contribution (positive charge)
    for k in range(N_i_count):
        pos_norm = x_i[k] * INV_DX_local
        p = int(math.floor(pos_norm))
        rmod = pos_norm - p
        
        if 0 <= p < N_G_local - 1:
            j_contrib_x = E_CHARGE_local * vx_i[k] * FACTOR_W
            j_contrib_y = E_CHARGE_local * vy_i[k] * FACTOR_W
            j_contrib_z = E_CHARGE_local * vz_i[k] * FACTOR_W
...
def do_one_cycle_jit_kernels(
    ...
    for t_loop_idx in range(N_T):
        ...
        # Step 2: Solve Poisson for electrostatic field Ex
        ...
        # ===================================================================
        # === FICTITIOUS ICP HEATING FIELD ===
        # Calculate the uniform, transverse heating field for this timestep
        E_y_induced_now = E_INDUCED_AMPLITUDE * math.sin(OMEGA_ICP * Time_arr_ref[0])
        # ===================================================================
        ...
        while k_e < num_electrons_to_process_this_timestep:
            ...
            vy_e[k_e] -= E_y_induced_now * FACTOR_E # Fictitious ICP heating
